...
// View for creating a lesson. This file contains a responsive desktop
// layout with form inputs for title/description, thumbnail upload,
// lesson materials upload, and one-or-more video links. The view uses
// GetX reactive primitives (Rx, Obx) to manage local UI state.
class AdminCreateLessonView extends GetResponsiveView {
  AdminCreateLessonView({super.key}) : super(alwaysUseBuilder: false);

  // Utilities and reactive state used by the view
  ...
  final RxList<Map<String, String>> videoLinks = <Map<String, String>>[
    {'link': '', 'title': ''},
  ].obs; // start with one empty link
  final ScrollController _videoLinksScrollController = ScrollController();
  final RxInt editingVideoIndex = (-1).obs; // -1 means not editing

  // Function to pick image for thumbnail. Uses image_picker to read an
  // image file from the gallery and stores its bytes in `selectedThumbnail`.
  ...
  // Adds a new video link pair (title + link) and scrolls the list so the
  // newly added item comes into view.
  ...
  // Deletes a video link at `index`. The UI requires at least one link, so
  // deletion is prevented when there's only a single item.
  ...
  // Toggle edit mode for a specific video link index. When editingVideoIndex
  // equals the index the UI shows save icon; otherwise it shows edit icon.
  ...
  // Pick lesson material files (non-media documents). Uses file_picker and
  // stores the selected files in `selectedFiles`.
  ...
  @override
  Widget phone() => const SizedBox.shrink(); // not used in this template
  @override
  Widget tablet() => const SizedBox.shrink(); // not used in this template

  @override
  Widget desktop() {
    // Desktop layout: sidebar + main content form
    return Scaffold(
      ...
      body: Row(
        children: [
          ...
          AdminSidebar(
            ...
            onLogout: () {
              // Navigate back to sign-in on logout
  ...
  Widget _buildLessonMaterialsUploadBox(
    BuildContext context,
    String title,
    String desc,
  ) {
      ...
      child: Column(
        ...
        children: [
          ...
          ElevatedButton(
            ...
            child: Obx(
              () => Text(selectedFiles.isNotEmpty ? "Change" : "Upload"),
            ),
          ),
        ],
      ),
